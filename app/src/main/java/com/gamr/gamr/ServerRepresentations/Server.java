...
public class Server {
    ...
    private static String post(String... params) throws IOException {
        ...
        System.out.println(text + " " + textInBytes.length);
        connection.setFixedLengthStreamingMode(textInBytes.length);

        //Open connection and write text
        BufferedOutputStream post = new BufferedOutputStream(connection.getOutputStream());
        post.write(textInBytes);

        return readResponseStream(connection);
    }

    private static String readResponseStream(HttpURLConnection connection) throws IOException {
